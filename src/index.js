...
app.get('/api/machines/:name/metrics/:key', async (req, res) => {
  ...
  if (since) {
    ...
    if (Number.isNaN(dt.getTime())) return res.status(400).json({ error: 'invalid_since' })
    where.createdAt = { [Op.gte]: dt }
  }
...
async function updateTemperatur(){
  ...
  for (const m of rows){
    ...
    await sequelize.query('UPDATE [Machines] SET temperatur = CAST(? AS DECIMAL(5,2)) WHERE id = ?', { replacements: [t, m.id] })
    const fresh = await Machine.findByPk(m.id)
    await emitTelemetry(fresh)
  }
}
...
async function updateLeistungUndGeschwindigkeit(){
  ...
  for (const m of rows){
    ...
    if (now - last >= PERSIST_EVERY_MS) {
      await sequelize.query('UPDATE [Machines] SET aktuelleLeistung = CAST(? AS DECIMAL(5,2)), geschwindigkeit = CAST(? AS DECIMAL(5,2)) WHERE id = ?', { replacements: [a, v, m.id] })
      const fresh = await Machine.findByPk(m.id)
      await emitTelemetry(fresh)
...
async function updateDurchlaufzeit(){
  ...
  for (const m of rows){
    ...
    await sequelize.query('UPDATE [Machines] SET durchgaengigeLaufzeit = CAST(? AS DECIMAL(12,3)) WHERE id = ?', { replacements: [d, m.id] })
  }
}
...
async function updateBetriebsminuten(){
  ...
  for (const m of rows){
    ...
    const last = lastPersist.get(m.id) || 0
    if (now - last >= PERSIST_EVERY_MS) {
      await sequelize.query('UPDATE [Machines] SET betriebsminutenGesamt = CAST(? AS DECIMAL(12,1)) WHERE id = ?', { replacements: [b, m.id] })
      const fresh = await Machine.findByPk(m.id)
      await emitTelemetry(fresh)
      lastPersist.set(m.id, now)
    } else {
...
async function updateLetzteWartung(){
  ...
  for (const m of rows){
    ...
    await sequelize.query('UPDATE [Machines] SET letzteWartung = ? WHERE id = ?', { replacements: [dateToDDMMYYYY(next), m.id] })
  }
}
